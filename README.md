...
3. **業務邏輯完整性**：
   ...
   - **物流整合**：模擬第三方 API（如 Shippo/FedEx），生成追蹤號碼並更新狀態，未來可擴展為多物流商路由選擇（如比價、最短配送時間）。
   - **實務價值**：確保庫存與訂單數據一致，減少倉庫錯誤，提升客戶滿意度。

4. **前後端分離與 API 設計**：
   - RESTful API (基路徑: `/api/wms`)，支援 GET/POST/PUT，包含資料驗證與錯誤處理。
   - 前端模擬倉庫操作介面（訂單建立、揀貨、分揀、包裝、出貨），透過 Axios 實現即時狀態刷新，方便 Demo 與操作展示。
...
3. **出貨策略**：
   - **邏輯**：模擬物流 API 整合（如 Shippo/FedEx），生成追蹤號碼並更新訂單狀態，支援錯誤處理與重試機制。
   - **優化方法**：未來可擴展為多物流商路由選擇，根據成本或配送時間動態選擇承運商。
   - **實務價值**：簡化出貨流程，確保物流追蹤準確，增強客戶體驗。

4. **整體流程最佳化**：
   - **狀態機設計**：使用有限狀態機 (Finite State Machine) 管理訂單狀態流轉，確保事件連鎖邏輯一致（如 Packed → Shipped 自動呼叫物流 API）。支援退貨、異常處理等擴展場景。
   - **效能優化**：資料表索引（`sku`, `order_number`）加速查詢；Redis 佇列分擔高併發壓力；系統支援水平擴展（Scale-out），可透過多個 Worker 處理佇列任務，適應高併發訂單場景。
...
6. **出貨整合**：
   ...
   - **觀察重點**：物流單號生成與狀態更新，模擬真實 API 整合流程，展示多物流商選擇潛力。
...
本專案不僅展示了 Laravel + Vue 的全棧開發能力，更融入了倉儲演算法（分批揀貨、分揀策略、物流路由）、事件驅動架構與非同步佇列，展現了工程思維與實務價值。從減少倉庫行走距離到支援高併發訂單處理，專案解決了真實電商痛點，並提供可擴展的架構（支援 ERP 整合、機器學習優化）。無論是面試展示、技術分享，或作為中小型電商倉庫的原型系統，都能快速上手並客製化。

## 🛠️ 常見問題與排錯

- **API 跨域問題**：檢查 Nginx CORS 配置，確保 `frontend/.env` 的 `VITE_API_BASE_URL` 正確。
- **佇列未執行**：檢查 Redis 容器狀態（`docker logs wms
